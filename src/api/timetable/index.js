 ...
 */
async function insertClassSchedulesInChunks(classes, chunkSize = 500, timetableId) {
  const chunks = Math.ceil(classes.length / chunkSize);
  console.log(`Processing ${classes.length} classes in ${chunks} chunks of ${chunkSize}`);
  ...
  for (let i = 0; i < classes.length; i += chunkSize) {
    ...
    try {
      ...
      throw new Error(`Failed to process timetable chunk ${chunkNumber}: ${error.message}`);
    }
  }
}
...
async function fetchAndCacheTimetable() {
  try {
    ...
    const classes = response.data.map(cls => ({
      ...
      day: new Date(cls.TIME_FROM_ISO).toLocaleDateString('en-US', { weekday: 'long' })
    }));
...
async function getEmptyRooms(date, [startTime, endTime]) {
  try {
    ...
    const occupiedRoomNumbers = new Set(occupiedRooms.map(r => r.roomNumber));
    const emptyRooms = allRooms
      .map(r => r.roomNumber)
      .filter(room => !occupiedRoomNumbers.has(room))
      .sort();
...
async function getRoomSchedule(roomNumber, date) {
  try {
    ...
    const classes = await prisma.classSchedule.findMany({
      where: {
        timetableId: latestTimetable.id,
        roomNumber: roomNumber.toUpperCase(), // Case-insensitive search
        startTime: {
          gte: startOfDay,
          lte: endOfDay
        }
